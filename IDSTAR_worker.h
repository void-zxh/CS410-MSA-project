#ifndef _IDASTARWORKER_H_
#define _IDASTARWORKER_H_
#include <iostream>
#include <cstdio>
#include <cstring>
#include <vector>
...
private:
    ...
    void IDAstar_dp_pre(char* query_x,int idx,char* query_y,int idy)
    {
    ...
    void ID_2_dfs(int x,int y,int g)
    {
        int h=get_2_H(len_x,x,y);
        //cout<<"ID_dfs "<<x<<' '<<y<<' '<<g<<endl;
        if(x==len_input&&y==len_x&&g<=ID_ans)
        {
            //cout<<"GET"<<endl;
            ID_flag=1;
            ID_ans=min(ID_ans,g);
        ...
        if(g+h>ID_ans)
        {
            LA_ans=max(g+h,LA_ans);
            return ;
        }
        if(x<len_input)
            ID_2_dfs(x+1,y,g+GAP);
        if(y<len_x)
            ID_2_dfs(x,y+1,g+GAP);
        if(x<len_input&&y<len_x)
        {
            if(query_input[x+1]==query_x[y+1])
                ID_2_dfs(x+1,y+1,g);
            else
                ID_2_dfs(x+1,y+1,g+MISMATCH);
        }
    }

    int IDAstar_2_step(char* query_comp,int id)
    {
        query_x=query_comp;
        len_x=len_data[id];
        ID_ans=get_2_H(len_x,0,0);
        ASTAR_node2 st(0,0,0,0),xi,yi;
        while(1)
        {
            ID_flag=0;
            LA_ans=ID_ans;
            cout<<"ID_ans: "<<ID_ans<<endl;
            ID_2_dfs(0,0,0);
            if(ID_flag)
                break;
            ID_ans=LA_ans;
    ...
    int IDAstar_3_step(char* query_x,int idx,char* query_y,int idy,int ID)
    {
        IDAstar_dp_pre(query_x,idx,query_y,idy);
...
public:
    ...
    void IDASTAR_2(char* input)
    {
        ...
        {
            //cout<<"NUM "<<i<<endl;
            re=IDAstar_2_step(MSA_data[i],i);
            cout<<"RE: "<<re<<endl;
            if(re<output_ans)
            {
                ans_id=i;
    ...
    void IDASTAR_3(char* input,int ID)
    {
        query_input=input;
        output_ans=inf;
        int re;
        ...
        for(int i=1;i<=data_size;i++)
            ...
            {
                //printf("%d %d\n",i,j);
                re=IDAstar_3_step(MSA_data[i],i,MSA_data[j],j,ID);
                if(re<output_ans)
                {
                    output_ans=re;
                    ans_idx=i; ans_idy=j;
